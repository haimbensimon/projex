...
import { globalNegative } from '../../../models/Simptom';
...
export class GlobalComponent implements OnInit {
  ...
  armsArr: globalNegative[] = [];

  armsArrFiltered: globalNegative[] = [];
  armsArrFiltered$!: Observable<globalNegative[]>;
  ...
  ngOnInit(): void {
    ...
    this.posService.globalP$.subscribe((post) => {
      this.armsArr = post;
    ...
    this.armsArrFiltered$.subscribe((p) => {
      p.sort((a: globalNegative, b: globalNegative) => {
  ...
  onDelteArms(pos: globalNegative) {
    this.posService.deleteglobal(pos).then((_) => {});
  }
  ...
  onSubmit() {
    const value: globalNegative = this.armsForm.value;
    this.posService.addglobal(value).then((_) => {
      this.router.navigate(['/preferences/global']);
    });
    this.armsForm.reset();
  }

  getpos(id: string) {
    this.posService.getglobal(id);
  }

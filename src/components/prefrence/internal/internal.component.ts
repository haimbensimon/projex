...
import { InternalNegative } from '../../../models/Simptom';
...
export class InternalComponent implements OnInit {
  ...
  armsArr: InternalNegative[] = [];

  armsArrFiltered: InternalNegative[] = [];
  armsArrFiltered$!: Observable<InternalNegative[]>;
  ...
  ngOnInit(): void {
    ...
    this.posService.internalP$.subscribe((post) => {
      this.armsArr = post;
    ...
    this.armsArrFiltered$.subscribe((p) => {
      p.sort((a: InternalNegative, b: InternalNegative) => {
  ...
  onDelteArms(pos: InternalNegative) {
    this.posService.deleteinternal(pos).then((_) => {});
  }
  ...
  onSubmit() {
    const value: InternalNegative = this.armsForm.value;
    this.posService.addinternal(value).then((_) => {
      this.router.navigate(['/preferences/internal']);
    });
    this.armsForm.reset();
  }

  getpos(id: string) {
    this.posService.getinternal(id);
  }
